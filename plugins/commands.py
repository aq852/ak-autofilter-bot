...
async def start(client, message):
    ...
    try:
        ...
        if len(m.command) == 2 and m.command[1].startswith(('notcopy', 'sendall')):
            ...
            shortener_number = verify_id_info.get("shortener_number")
            if stage == 2:
        ...
        if not await db.has_premium_access(user_id):
            try:
                ...
                settings = await get_settings(grp_id)
                active_shorteners = max(1, min(3, int(settings.get('active_shorteners', 3))))
                enabled_shorteners = list(range(1, active_shorteners + 1))
                verification_stage = None
                if enabled_shorteners and settings.get("is_verify", IS_VERIFY):
                    if not user_verified:
                        verification_stage = 0
                    elif len(enabled_shorteners) >= 2 and await db.use_second_shortener(
                        user_id, settings.get('verify_time', TWO_VERIFY_GAP)
                    ):
                        verification_stage = 1
                    elif len(enabled_shorteners) >= 3 and await db.use_third_shortener(
                        user_id, settings.get('third_verify_time', THREE_VERIFY_GAP)
                    ):
                        verification_stage = 2

                if verification_stage is not None:
                    shortener_number = enabled_shorteners[verification_stage]
                    verify_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=7))
                    await db.create_verify_id(user_id, verify_id, verification_stage, shortener_number)
                    temp.VERIFICATIONS[user_id] = grp_id
                    if message.command[1].startswith('allfiles'):
                        verify = await get_shortlink(f"https://telegram.me/{temp.U_NAME}?start=sendall_{user_id}_{verify_id}_{file_id}", grp_id, shortener_number)
                    else:
                        verify = await get_shortlink(f"https://telegram.me/{temp.U_NAME}?start=notcopy_{user_id}_{verify_id}_{file_id}", grp_id, shortener_number)
                    tutorials = {
                        1: settings.get('tutorial', TUTORIAL),
                        2: settings.get('tutorial_2', TUTORIAL_2),
                        3: settings.get('tutorial_3', TUTORIAL_3),
                    }
                    howtodownload = tutorials[shortener_number]
                    buttons = [[
                    ...
                    if verification_stage == 2:
                        ...
                        msg = script.SECOND_VERIFICATION_TEXT if verification_stage == 1 else script.VERIFICATION_TEXT
...
async def reset_group_callback(client, callback_query):
    ...
    defaults = {
        ...
        'shortner_1_enabled': True,
        'shortner_2_enabled': True,
        'shortner_3_enabled': True,
        'active_shorteners': 3,
...
@Client.on_message(filters.command("shortener_count"))
async def set_shortener_count(client, message):
    if message.chat.type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        return await message.reply_text("Use this command inside the group.")
    if not await is_check_admin(client, message.chat.id, message.from_user.id):
        return await message.reply_text(script.NT_ADMIN_ALRT_TXT)
    try:
        count = int(message.text.split(maxsplit=1)[1])
    except (IndexError, ValueError):
        return await message.reply_text("Usage: <code>/shortener_count 1</code>, <code>2</code>, or <code>3</code>")
    if count not in (1, 2, 3):
        return await message.reply_text("Choose 1, 2, or 3 active shorteners.")
    await save_group_settings(message.chat.id, 'active_shorteners', count)
    await message.reply_text(f"This group will use {count} shortener stage(s): 1 through {count}.")

@Client.on_message(filters.command("global_verify_time") & filters.user(ADMINS))
async def set_global_verify_time(client, message):
    parts = message.text.split()
    if len(parts) == 2 and parts[1].lower() == "reset":
        await mdb.clear_global_verify_gaps()
        temp.GLOBAL_VERIFY_GAPS = None
        temp.GLOBAL_VERIFY_GAPS_LOADED = True
        return await message.reply_text("Global verification timing removed; group timing is active again.")
    if len(parts) != 3:
        return await message.reply_text(
            "Usage: <code>/global_verify_time 1200 54000</code>\n"
            "Values are seconds: first-to-second, then second-to-third."
        )
    try:
        two_gap, three_gap = int(parts[1]), int(parts[2])
    except ValueError:
        return await message.reply_text("Both timing values must be numbers in seconds.")
    if two_gap < 0 or three_gap < 0:
        return await message.reply_text("Timing values cannot be negative.")
    await mdb.set_global_verify_gaps(two_gap, three_gap)
    temp.GLOBAL_VERIFY_GAPS = (two_gap, three_gap)
    temp.GLOBAL_VERIFY_GAPS_LOADED = True
    await message.reply_text(f"Global verification timing set: {two_gap}s then {three_gap}s.")

@Client.on_message(filters.command("global_shortener") & filters.user(ADMINS))
async def set_global_shortener_mode(client, message):
    """Set one verification-shortener combination for every group."""
    parts = message.text.split(maxsplit=1)
    if len(parts) < 2:
        return await message.reply_text(
            "Usage: <code>/global_shortener 1</code>\n"
            "Examples: <code>1</code>, <code>1,2</code>, <code>2,3</code>, <code>1,2,3</code>\n"
            "Use <code>/global_shortener reset</code> to return control to each group."
        )

    option = parts[1].strip().lower()
    if option == "reset":
        await mdb.clear_global_shortener_stages()
        temp.GLOBAL_SHORTENER_STAGES = None
        temp.GLOBAL_SHORTENER_STAGES_LOADED = True
        return await message.reply_text(
            "Global shortener mode removed. Each group can now manage its own shorteners."
        )

    try:
        stages = sorted({int(value) for value in re.split(r"[\s,]+", option) if value})
    except ValueError:
        stages = []
    if not stages or any(stage not in (1, 2, 3) for stage in stages):
        return await message.reply_text(
            "Use only 1, 2, or 3. For example: <code>/global_shortener 1,2</code>"
        )
    if stages != list(range(1, len(stages) + 1)):
        return await message.reply_text("Use a count-based mode: 1, 1,2, or 1,2,3.")

    await mdb.set_global_shortener_stages(stages)
    temp.GLOBAL_SHORTENER_STAGES = stages
    temp.GLOBAL_SHORTENER_STAGES_LOADED = True
    enabled = ", ".join(f"Shortener {stage}" for stage in stages)
    disabled = ", ".join(f"Shortener {stage}" for stage in (1, 2, 3) if stage not in stages)
    await message.reply_text(
        f"Global verification mode updated.\n\nEnabled for every group: <b>{enabled}</b>\n"
        f"Disabled: <b>{disabled or 'None'}</b>"
    )
