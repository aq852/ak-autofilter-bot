...
class Database:    
    ...
    async def get_settings(self, id):
        default = {
            ...
            # Each verification stage can be controlled independently.
            # Defaults preserve the existing three-stage behaviour for all groups.
    ...
    async def use_third_shortener(self, user_id, time, after_second_stage=True):
        user = await self.get_notcopy_user(user_id)
        if not user.get("third_time_verified"):
            ...
            user = await self.get_notcopy_user(user_id)
        is_ready_for_third = await self.user_verified(user_id) if after_second_stage else await self.is_user_verified(user_id)
        if is_ready_for_third:
            verification_key = "second_time_verified" if after_second_stage else "last_verified"
            try:
                pastDate = user[verification_key]
            except Exception:
                user = await self.get_notcopy_user(user_id)
                pastDate = user[verification_key]
            ist_timezone = pytz.timezone('Asia/Kolkata')
            ...
            time_difference = current_time - pastDate
            if time_difference > datetime.timedelta(seconds=time):
                pastDate = user[verification_key].astimezone(ist_timezone)
                second_time = user["third_time_verified"].astimezone(ist_timezone)
                return second_time < pastDate
        return False
   
    async def create_verify_id(self, user_id: int, hash, stage=None, shortener_number=None):
        res = {
            "user_id": user_id,
            "hash": hash,
            "verified": False,
            "stage": stage,
            "shortener_number": shortener_number,
        }
