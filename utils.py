...
class temp(object):   
    ...
    TEMP_INVITE_LINKS = {}
    REQ_LINKS = {}
    GLOBAL_SHORTENER_STAGES = None
    GLOBAL_SHORTENER_STAGES_LOADED = False
    GLOBAL_VERIFY_GAPS = None
    GLOBAL_VERIFY_GAPS_LOADED = False
...
async def get_shortlink(link, grp_id, shortener_number=1):
    settings = await get_settings(grp_id)
    if shortener_number == 3:
        api, site = settings['api_three'], settings['shortner_three']
    elif shortener_number == 2:
...
async def get_settings(group_id):
    ...
    if not temp.GLOBAL_SHORTENER_STAGES_LOADED:
        temp.GLOBAL_SHORTENER_STAGES = await mdb.get_global_shortener_stages()
        temp.GLOBAL_SHORTENER_STAGES_LOADED = True
    if not temp.GLOBAL_VERIFY_GAPS_LOADED:
        temp.GLOBAL_VERIFY_GAPS = await mdb.get_global_verify_gaps()
        temp.GLOBAL_VERIFY_GAPS_LOADED = True

    # When the owner sets a global mode, it intentionally overrides each
    # group's individual selection while preserving all other group settings.
    if temp.GLOBAL_SHORTENER_STAGES is not None:
        settings = settings.copy()
        settings['active_shorteners'] = len(temp.GLOBAL_SHORTENER_STAGES)
    if temp.GLOBAL_VERIFY_GAPS is not None:
        settings = settings.copy()
        settings['verify_time'], settings['third_verify_time'] = temp.GLOBAL_VERIFY_GAPS
    return settings
    
async def save_group_settings(group_id, key, value):
    group_id = int(group_id)
    # Save the group's own settings, not the globally-overridden view returned
    # by get_settings(). This preserves its choices if the owner later resets
    # global shortener mode.
    current = temp.SETTINGS.get(group_id)
    if not current:
